...
-- DESTINO_TICKET [rel1]
alter table `viaje`  add column  `destino_2_oid`  integer;
alter table `viaje`   add index fk_viaje_destino_2 (`destino_2_oid`), add constraint fk_viaje_destino_2 foreign key (`destino_2_oid`) references `destino_2` (`oid`);


-- TIPO_VIAJE_TICKET [rel2]
alter table `viaje`  add column  `tipo_viaje_2_oid`  integer;
alter table `viaje`   add index fk_viaje_tipo_viaje_2 (`tipo_viaje_2_oid`), add constraint fk_viaje_tipo_viaje_2 foreign key (`tipo_viaje_2_oid`) references `tipo_viaje_2` (`oid`);


-- User_TICKET [rel3]
alter table `viaje`  add column  `user_2_oid`  integer;
alter table `viaje`   add index fk_viaje_user_2 (`user_2_oid`), add constraint fk_viaje_user_2 foreign key (`user_2_oid`) references `user_2` (`oid`);
...
-- VIAJE_TICKET [rel7]
alter table `ticket_3`  add column  `viaje_oid`  integer;
alter table `ticket_3`   add index fk_ticket_3_viaje (`viaje_oid`), add constraint fk_ticket_3_viaje foreign key (`viaje_oid`) references `viaje` (`oid`);
